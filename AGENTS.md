...
Standard commands live in the root `package.json` scripts (`dev`, `lint`, `test`, `build`, `mcp:mock`); CI runs them in `.github/workflows/ci.yml`. Dependencies are installed on startup by the update script (`npm ci`), which also builds `@conviction/mcp` via its `prepare` script.
...
- **Two independent surfaces.** The Next.js web app and the `@conviction/mcp` stdio server are launched separately; neither requires the other. `npm run mcp:mock` runs the MCP server in deterministic mock mode (stdio JSON-RPC, no web app, no credentials).
- **`npm test` rebuilds `@conviction/mcp` first** (`tsc`) before running Vitest; run it from the repo root.
