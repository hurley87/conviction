...
/** True once Particle finality is terminal and no read-only retry can advance it. */
export function isProviderTerminalOutcome(outcome: ExecutionOutcome): boolean {
  return (
    outcome === "finalized" ||
    outcome === "partial" ||
    outcome === "failed" ||
