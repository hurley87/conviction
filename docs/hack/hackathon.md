...
- [x] **The spine proof is a manual in-app run, not a script.** Run the real flow once in the deployed app with the desk wallet — 7702 upgrade → deposits from ≥2 chains unify → cross-chain trade — and record the **receipt permalink** as the artifact (ADR 0013: the receipt *is* the verification artifact). `scripts/smoke-spine.ts` is the pre-flight only (env + unified balance + quote shape + cross-chain check); it does not and should not execute — no demo-week time in the 10% category.
  - Receipt: http://localhost:3000/r/10d7aa2987f4 (Base + Arbitrum legs; desk ETH ~$20 settled on Arbitrum). Hero card published as `e49fc42f6fb180b8` (@davidbhurley) with entry onchain at 2026-07-15T18:20:44Z before publish.
